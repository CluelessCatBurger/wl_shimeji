...
// Usefull macros
#define pthread_scoped_lock(name, mutex)                                       \
  __attribute__((cleanup(pthread_scope_unlock)))                               \
  pthread_mutex_t *__scope_lock##name = mutex;                                 \
...
#define environment_by_global_coords(x, y)                                     \
  lookup_environment_by_coords ? lookup_environment_by_coords(x, y) : NULL
...
struct environment {
  ...
  bool select_active;
  void (*select_callback)(environment_t *env, int32_t x, int32_t y,
                          environment_subsurface_t *subsurface, void *data);
  void *select_data;
...
// Primary button, usually left mouse button, on tablets and touchscreens
// corresponds to the down event
...
struct environment_pointer {
  ...
  // Public position of the pointer, in global coordinates, exposed to mascots,
  // only by plugins
...
struct active_ie active_ie = {.is_active = false};
...
environment_subsurface_t *
environment_subsurface_from_surface(struct wl_surface *surface) {
  if (!surface)
    return NULL;
  struct wl_surface_data *data = wl_surface_get_user_data(surface);
  if (!data)
    return NULL;
  if (data->role != WL_SURFACE_ROLE_SUBSURFACE)
    return NULL;
...
struct layer_surface *layer_surface_from_surface(struct wl_surface *surface) {
  ...
  if (!data)
    return NULL;
  if (data->role != WL_SURFACE_ROLE_LAYER_COMPONENT &&
      data->role != WL_SURFACE_ROLE_PARENT_SURFACE)
    return NULL;
  return data->role_data.layer_surface;
}

struct wl_surface_data *wl_surface_set_data(struct wl_surface *surface,
                                            uint8_t role, void *data,
                                            environment_t *env) {
  if (!surface)
    return NULL;
  ...
  } else if (role == WL_SURFACE_ROLE_LAYER_COMPONENT ||
             role == WL_SURFACE_ROLE_PARENT_SURFACE) {
...
void enable_input_on_environments(bool enable) {
  if (enable) {
    ...
    int32_t oldval =
        __atomic_fetch_add(&active_grabbers_count, 1, __ATOMIC_SEQ_CST);
    if (!oldval) {
    ...
    int32_t oldval =
        __atomic_fetch_sub(&active_grabbers_count, 1, __ATOMIC_SEQ_CST);
    if (oldval) {
      if (env_broadcast_input_enabled) {
...
bool environment_pointer_apply_cursor(environment_pointer_t *pointer,
                                      int32_t cursor_type) {
  if (!pointer)
    return false;
  if (!pointer->cursor_shape_device && !cursor_theme)
    return false;
  ...
  if (pointer->cursor_shape_device) {
    wp_cursor_shape_device_v1_set_shape(pointer->cursor_shape_device,
                                        pointer->enter_serial, cursor_shape);
  ...
  if (pointer->protocol == ENVIRONMENT_POINTER_PROTOCOL_POINTER) {
    wl_pointer_set_cursor(pointer->device.pointer, pointer->enter_serial,
                          pointer->above_surface, image->hotspot_x,
                          image->hotspot_y);
  } else if (pointer->protocol == ENVIRONMENT_POINTER_PROTOCOL_TABLET) {
    zwp_tablet_tool_v2_set_cursor(pointer->device.tablet, pointer->enter_serial,
                                  pointer->above_surface, image->hotspot_x,
                                  image->hotspot_y);
  }
...
environment_pointer_t *allocate_env_pointer(int32_t protocol,
                                            void *proxy_object) {
  environment_pointer_t *pointer =
      (environment_pointer_t *)calloc(1, sizeof(environment_pointer_t));
  if (!pointer)
    ERROR("Failed to allocate memory for environment pointer");
  ...
  if (protocol == ENVIRONMENT_POINTER_PROTOCOL_POINTER) {
    ...
    if (cursor_shape_manager) {
      pointer->cursor_shape_device = wp_cursor_shape_manager_v1_get_pointer(
          cursor_shape_manager, (struct wl_pointer *)proxy_object);
    ...
    if (cursor_shape_manager) {
      pointer->cursor_shape_device =
          wp_cursor_shape_manager_v1_get_tablet_tool_v2(
              cursor_shape_manager, (struct zwp_tablet_tool_v2 *)proxy_object);
      pointer->current_shape = WP_CURSOR_SHAPE_DEVICE_V1_SHAPE_DEFAULT;
    }
...
static void block_until_synced() { wl_display_roundtrip(display); }

// WTF WAS THAT? it basically was doing same thing then calling
// wl_display_roundtrip for literally no reason at all. Also Niri was broken
// because of it. static void block_until_synced() {
//   bool lock = false;
...
static void environment_dimensions_changed_callback(uint32_t width,
                                                    uint32_t height,
                                                    void *data) {
  ...
  for (uint32_t i = 0; i < list_size(env->mascot_manager.referenced_mascots);
       i++) {
    struct mascot *mascot = list_get(env->mascot_manager.referenced_mascots, i);
    if (mascot) {
      if (mascot->Y->value.i == 0 ||
          mascot->Y->value.i == env->advertised_geometry.height) {
...
static void mascot_on_pointer_enter(void *data, struct wl_pointer *pointer,
                                    uint32_t serial, struct wl_surface *surface,
                                    wl_fixed_t x, wl_fixed_t y);
static void mascot_on_pointer_button(void *data, struct wl_pointer *pointer,
                                     uint32_t serial, uint32_t time,
                                     uint32_t button, uint32_t state);
static void mascot_on_pointer_axis(void *data, struct wl_pointer *pointer,
                                   uint32_t time, uint32_t axis,
                                   wl_fixed_t value);
static void mascot_on_pointer_leave(void *data, struct wl_pointer *pointer,
                                    uint32_t serial,
                                    struct wl_surface *surface);
static void mascot_on_pointer_motion(void *data, struct wl_pointer *pointer,
...
static void environment_on_pointer_enter(void *data, struct wl_pointer *pointer,
                                         uint32_t serial,
                                         struct wl_surface *surface,
                                         wl_fixed_t x, wl_fixed_t y);
static void environment_on_pointer_button(void *data,
                                          struct wl_pointer *pointer,
                                          uint32_t serial, uint32_t time,
                                          uint32_t button, uint32_t state);
static void environment_on_pointer_motion(void *data,
                                          struct wl_pointer *pointer,
                                          uint32_t time, wl_fixed_t x,
                                          wl_fixed_t y);
static void environment_on_pointer_leave(void *data, struct wl_pointer *pointer,
                                         uint32_t serial,
                                         struct wl_surface *surface);
...
static void on_preffered_scale(void *data,
                               struct wp_fractional_scale_v1 *fractional_scale,
                               uint32_t scale);

static const struct wp_fractional_scale_v1_listener
    fractional_scale_manager_v1_listener = {
...
enum environment_init_status dispatch_envs_queue(struct envs_queue *envs) {
  for (size_t i = 0; i < envs->envs_count; i++) {
    ...
    env->root_surface =
        layer_surface_create(env->output.output, config_get_overlay_layer());
    if (!env->root_surface) {
      WARN("FAILED TO CREATE LAYER SURFACE ON OUTPUT %u aka %s", env->id,
           env->output.name);
      if (rem_environment) {
        rem_environment(env);
      }
    ...
    layer_surface_set_dimensions_callback(
        env->root_surface, environment_dimensions_changed_callback, env);
    layer_surface_set_closed_callback(env->root_surface,
                                      environment_wants_to_close_callback, env);
    layer_surface_map(env->root_surface);
    block_until_synced();
    wl_surface_set_data(env->root_surface->surface,
                        WL_SURFACE_ROLE_PARENT_SURFACE,
                        (void *)env->root_surface, env);
    if (env->root_surface->configure_serial) {
      env->is_ready = true;
      env->root_environment_subsurface = environment_create_subsurface(env);
      wl_surface_set_data(env->root_environment_subsurface->surface,
                          WL_SURFACE_ROLE_LAYER_COMPONENT, env->root_surface,
                          env);

      env->scale = env->output.scale;

      if (fractional_manager) {
        env->root_environment_subsurface->fractional_scale =
            wp_fractional_scale_manager_v1_get_fractional_scale(
                fractional_manager, env->root_environment_subsurface->surface);
        if (!env->root_environment_subsurface->fractional_scale) {
          ...
          error_offt += snprintf(error_m + error_offt, 1024 - error_offt,
                                 "Failed to get fractional scale for root "
        ...
        wp_fractional_scale_v1_add_listener(
            env->root_environment_subsurface->fractional_scale,
            &fractional_scale_manager_v1_listener, env);
      }

      if (xdg_output_manager) {
        env->xdg_output = zxdg_output_manager_v1_get_xdg_output(
            xdg_output_manager, env->output.output);
        if (!env->xdg_output) {
          wl_surface_clear_data(env->root_environment_subsurface->surface);
          wl_surface_destroy(env->root_environment_subsurface->surface);
          free(env->root_environment_subsurface);
          error_offt += snprintf(
              error_m + error_offt, 1024 - error_offt,
              "Failed to get xdg output for root environment subsurface\n");
          init_status = ENV_INIT_ERROR_GENERIC;
          return ENV_INIT_ERROR_GENERIC;
        }
      ...
      struct environment_callbacks *callbacks =
          (struct environment_callbacks *)calloc(
              1, sizeof(struct environment_callbacks));
      if (!callbacks) {
        error_offt +=
            snprintf(error_m + error_offt, 1024 - error_offt,
                     "Failed to allocate memory for environment callbacks\n");
      ...
      wl_surface_attach_callbacks(env->root_surface->surface,
                                  (void *)callbacks);
      wl_surface_attach(env->root_environment_subsurface->surface,
                        anchor_buffer, 0, 0);
    }
...
static const struct wl_shm_listener wl_shm_listener_cb = {.format = on_format};
...
static void on_pointer_frame(void *data, struct wl_pointer *pointer) {
  ...
  env_subsurface =
      environment_subsurface_from_surface(env_pointer->above_surface);
  surface_pointer_callbacks =
      wl_surface_get_callbacks(env_pointer->above_surface);

  // Workaround for hyprland
  if (env_pointer->grabbed_subsurface) {
    if ((env_pointer->frame.mask &
         (EVENT_FRAME_BUTTONS | EVENT_FRAME_SURFACE)) ==
        (EVENT_FRAME_BUTTONS | EVENT_FRAME_SURFACE)) {
      if (env_pointer->above_surface ==
              env_pointer->grabbed_subsurface->surface &&
          env_pointer->frame.surface_changed) {
        env_pointer->frame.mask &= ~EVENT_FRAME_BUTTONS;
      }
    }
  }
  ...
  if (env_pointer->frame.mask & EVENT_FRAME_SURFACE) {
    if (surface_pointer_callbacks) {
      if (surface_pointer_callbacks->pointer_listener) {
        if (surface_pointer_callbacks->pointer_listener->leave) {
          surface_pointer_callbacks->pointer_listener->leave(
              (void *)env_pointer, NULL, env_pointer->enter_serial,
              env_pointer->above_surface);
        }
      }
    }
    ...
    environment_t *new_env =
        environment_from_surface(env_pointer->above_surface);
    env_subsurface =
        environment_subsurface_from_surface(env_pointer->above_surface);
    surface_pointer_callbacks =
        wl_surface_get_callbacks(env_pointer->above_surface);
    if (new_env && new_env != env && env_pointer->grabbed_subsurface) {
      struct mascot *mascot =
          environment_subsurface_get_mascot(env_pointer->grabbed_subsurface);
      if (mascot && (config_get_allow_dragging_multihead() ||
        ...
        mascot_apply_environment_position_diff(
            mascot, diffx, diffy, DIFF_HORIZONTAL_MOVE | DIFF_VERTICAL_MOVE,
            new_env);
    ...
    if (new_env)
      env_pointer->above_environment = new_env;

    // Finally, send the enter callback for the new surface
    if (surface_pointer_callbacks) {
      if (surface_pointer_callbacks->pointer_listener) {
        if (surface_pointer_callbacks->pointer_listener->enter) {
          surface_pointer_callbacks->pointer_listener->enter(
              (void *)env_pointer, NULL, env_pointer->enter_serial,
              env_pointer->above_surface, env_pointer->frame.surface_local_x,
              env_pointer->frame.surface_local_y);
        }
      }
    }
  }
    ...
    if (surface_pointer_callbacks) {
      if (surface_pointer_callbacks->pointer_listener) {
        if (surface_pointer_callbacks->pointer_listener->motion) {
          surface_pointer_callbacks->pointer_listener->motion(
              (void *)env_pointer, NULL, 0, env_pointer->frame.surface_local_x,
              env_pointer->frame.surface_local_y);
        }
      }
    }
    env_pointer->surface_x =
        wl_fixed_to_int(env_pointer->frame.surface_local_x);
    env_pointer->surface_y =
        wl_fixed_to_int(env_pointer->frame.surface_local_y);
  ...
  if (env_pointer->frame.mask & EVENT_FRAME_BUTTONS) {
    ...
    if (!env_pointer->above_surface && env_pointer->grabbed_subsurface) {
      // Impossible situation, but in case we somehow got button release after
      // leave(), we should handle it
      surface_pointer_callbacks =
          wl_surface_get_callbacks(env_pointer->grabbed_subsurface->surface);
    ...
    for (int i = 0; i < 4; i++) {
      int32_t button = 1 << i;
      if ((button & env_pointer->frame.buttons_released) &&
          (button & env_pointer->buttons_state)) {
        if (surface_pointer_callbacks) {
          if (surface_pointer_callbacks->pointer_listener) {
            if (surface_pointer_callbacks->pointer_listener->button) {
              surface_pointer_callbacks->pointer_listener->button(
                  (void *)env_pointer, NULL, 0, 0, button,
                  WL_POINTER_BUTTON_STATE_RELEASED);
            }
          }
        }
        env_pointer->buttons_state &= ~button;
      } else if ((button & env_pointer->frame.buttons_pressed) &&
                 !(button & env_pointer->buttons_state)) {
        if (surface_pointer_callbacks) {
          if (surface_pointer_callbacks) {
            if (surface_pointer_callbacks->pointer_listener) {
    ...
    if (surface_pointer_callbacks) {
      if (surface_pointer_callbacks->pointer_listener) {
        if (surface_pointer_callbacks->pointer_listener->axis) {
          surface_pointer_callbacks->pointer_listener->axis((void *)env_pointer,
                                                            NULL, 0, 0, 0);
        }
      }
    }
...
static void on_pointer_axis_value120(void *data, struct wl_pointer *pointer,
...
static void on_pointer_axis_relative_direction(void *data,
                                               struct wl_pointer *pointer,
                                               uint32_t axis,
                                               uint32_t direction) {
  UNUSED(data);
...
static void mascot_on_pointer_enter(void *data, struct wl_pointer *pointer,
  ...
  environment_subsurface_t *env_surface =
      environment_subsurface_from_surface(surface);
  if (!env_surface) {
    WARN("Unexpected lack of env_subsurface while we are in "
         "mascot_on_pointer_enter!");
    return;
  }
...
static void mascot_on_pointer_motion(void *data, struct wl_pointer *pointer,
  ...
  if (!env_surface) {
    WARN("Unexpected lack of env_subsurface while we are in "
         "mascot_on_pointer_motion!");
    return;
  }

  struct mascot *mascot = environment_subsurface_get_mascot(env_surface);
  if (!mascot) {
    WARN("Unexpected! mascot_on_pointer_motion is called, but somehow we not "
         "above mascot???");
    return;
  }
  ...
  // global_x = env_surface->pose->anchor_x + wl_fixed_to_int(x) +
  // env_surface->mascot->X->value.i; global_y = env_surface->pose->anchor_y +
  // wl_fixed_to_int(y) + env_surface->mascot->Y->value.i;

  bool is_dragged_surface = false;
  if (env_pointer->grabbed_subsurface) {
    if (env_pointer->grabbed_subsurface->surface ==
        env_pointer->above_surface) {
  ...
  if (is_dragged_surface) {
    ...
    global_x = mascot->X->value.i + wl_fixed_to_int(x) +
               anchor_x / (config_get_mascot_scale() * env_surface->env->scale);
    global_y = yconv(env_surface->env, mascot->Y->value.i) +
               wl_fixed_to_int(y) +
               anchor_y / (config_get_mascot_scale() * env_surface->env->scale);
  }
  ...
  // Get hotspot
  int32_t scaled_x = wl_fixed_to_int(env_pointer->frame.surface_local_x),
          scaled_y = wl_fixed_to_int(env_pointer->frame.surface_local_y);
  environment_subsurface_scale_coordinates(env_surface, &scaled_x, &scaled_y);
  struct mascot_hotspot *hotspot =
      mascot_hotspot_by_pos(env_surface->mascot, scaled_x, scaled_y);
  if (hotspot && !(env_pointer->grabbed_subsurface || env->select_active))
    environment_pointer_apply_cursor(env_pointer, hotspot->cursor);
  else if (!hotspot && !(env_pointer->grabbed_subsurface || env->select_active))
...
static void environment_on_pointer_button(void *data,
  ...
  environment_t *env = environment_from_surface(env_pointer->above_surface);
  if (!env) {
    WARN("Unexpected lack of env while we are in "
         "environment_on_pointer_button!");
    return;
  }
  ...
  if (state == WL_POINTER_BUTTON_STATE_PRESSED) {
    if (button == EVENT_FRAME_PRIMARY_BUTTON) {
      if (env->select_active) {
        ...
        env->select_callback(
            env, env_pointer->x, yconv(env, env_pointer->y),
            environment_subsurface_from_surface(env_pointer->above_surface),
            env->select_data);
        environment_pointer_apply_cursor(env_pointer, -2);
      }
...
static void mascot_on_pointer_button(void *data, struct wl_pointer *pointer,
  ...
  if (state == WL_POINTER_BUTTON_STATE_PRESSED) {
    if (!env->select_active && !env_pointer->grabbed_subsurface) {
      bool hotspot_press_result = mascot_hotspot_hold(
          mascot, scaled_x, scaled_y, pressed_button, false);
      if (!hotspot_press_result &&
          pressed_button == mascot_hotspot_button_left) {
        mascot_drag_started(mascot, env_pointer);
      }
      if (!hotspot_press_result &&
          pressed_button == mascot_hotspot_button_right) {
        protocol_server_announce_new_click(mascot, env_pointer->x,
                                           env_pointer->y);
      }
    } else {
    ...
    if (env_pointer->grabbed_subsurface &&
      ...
      if (mascot->hotspot_active || env_pointer->grabbed_subsurface) {
        ...
        struct mascot_hotspot *hotspot =
            mascot_hotspot_by_pos(mascot, scaled_x, scaled_y);
        if (hotspot) {
          environment_pointer_apply_cursor(env_pointer, hotspot->cursor);
        } else {
          environment_pointer_apply_cursor(env_pointer, -2);
        ...
        environment_pointer_apply_cursor(env_pointer, -2);
        environment_on_pointer_button(data, pointer, serial, time, button,
                                      state);
      }
    }
  }
...
static void mascot_on_pointer_axis(void *data, struct wl_pointer *pointer,
  ...
  if (env_surface) {
    mascot_hotspot_click(env_surface->mascot, env_pointer->surface_x,
                         env_pointer->surface_y, mascot_hotspot_button_middle);
  }
}
...
static void environment_on_pointer_enter(void *data, struct wl_pointer *pointer,
  ...
  if (!env) {
    WARN(
        "Unexpected lack of env while we are in environment_on_pointer_enter!");
    return;
  }
...
static void environment_on_pointer_leave(void *data, struct wl_pointer *pointer,
  ...
  if (!env) {
    WARN(
        "Unexpected lack of env while we are in environment_on_pointer_leave!");
    return;
  }
}
...
static void on_seat_capabilities(void *data, struct wl_seat *seat,
  ...
  if (capabilities & WL_SEAT_CAPABILITY_POINTER) {
    ...
    environment_pointer_t *env_pointer =
        allocate_env_pointer(ENVIRONMENT_POINTER_PROTOCOL_POINTER, pointer);
  ...
  if (capabilities & WL_SEAT_CAPABILITY_TOUCH) {
    ...
    // environment_pointer_t* env_pointer =
    // allocate_env_pointer(ENVIRONMENT_POINTER_PROTOCOL_TOUCH, touch);
...
static void on_tool_type(void *data, struct zwp_tablet_tool_v2 *tool,
                         uint32_t type) {
  UNUSED(tool);
  environment_pointer_t *env_pointer = (environment_pointer_t *)data;
  if (!env_pointer) {
...
static void on_tool_hardware_serial(void *data, struct zwp_tablet_tool_v2 *tool,
...
static void on_tool_hardware_id_wacom(void *data,
...
static void on_tool_capability(void *data, struct zwp_tablet_tool_v2 *tool,
                               uint32_t capability) {
  UNUSED(data);
  UNUSED(tool);
  UNUSED(capability);
...
static void on_tool_removed(void *data, struct zwp_tablet_tool_v2 *tool) {
  ...
  struct wl_pointer_listener *surface_pointer_callbacks =
      wl_surface_get_callbacks(env_pointer->above_surface);
  ...
  if (surface_pointer_callbacks) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (button & env_pointer->buttons_state) {
        if (surface_pointer_callbacks->button) {
          surface_pointer_callbacks->button((void *)env_pointer, NULL, 0, 0,
                                            button,
                                            WL_POINTER_BUTTON_STATE_RELEASED);
        }
      }
    ...
    if (surface_pointer_callbacks->leave) {
      surface_pointer_callbacks->leave((void *)env_pointer, NULL,
                                       env_pointer->enter_serial,
                                       env_pointer->above_surface);
    }
  }
...
static void on_tool_proximity_in(void *data, struct zwp_tablet_tool_v2 *tool,
...
static void on_tool_down(void *data, struct zwp_tablet_tool_v2 *tool,
                         uint32_t serial) {
  UNUSED(tool);
...
static void on_tool_button(void *data, struct zwp_tablet_tool_v2 *tool,
...
static void on_tool_motion(void *data, struct zwp_tablet_tool_v2 *tool,
                           wl_fixed_t x, wl_fixed_t y) {
  UNUSED(tool);
  ...
  //     if (!env_pointer->grabbed_subsurface) {
  //         // Now our coordinates is in global space, we to map them back to
  //         surface space int32_t global_x = wl_fixed_to_int(x); int32_t
  //         global_y = wl_fixed_to_int(y);

  //         int32_t local_x;
  //         int32_t local_y;

  //         environment_t* env =
  //         environment_from_surface(env_pointer->above_surface); if (!env)
  //         return;

  //         struct environment_subsurface* env_subsurface =
  //         environment_subsurface_from_surface(env_pointer->above_surface); if
  //         (!env_subsurface) return;
  ...
  //         local_x = global_x - anchor_x * (config_get_mascot_scale() *
  //         env->scale) - mascot->X->value.i; local_y = global_y - anchor_y *
  //         (config_get_mascot_scale() * env->scale) - yconv(env,
  //         mascot->Y->value.i);
  ...
  //         environment_t* env =
  //         environment_from_surface(env_pointer->above_surface); if (!env)
  //         return; if (env_pointer->above_surface ==
  //         env_pointer->grabbed_subsurface->surface) {
  //             env_pointer->frame.mask |= EVENT_FRAME_SURFACE;
  //             env_pointer->frame.surface_changed =
  //             env->root_surface->surface; env_pointer->frame.enter_serial =
  //             env_pointer->enter_serial;
  //         }
  //     }
  // }
...
static void on_tool_pressure(void *data, struct zwp_tablet_tool_v2 *tool,
                             uint32_t pressure) {
  UNUSED(data);
  UNUSED(tool);
  UNUSED(pressure);
...
static void on_tool_distance(void *data, struct zwp_tablet_tool_v2 *tool,
                             uint32_t distance) {
  UNUSED(data);
  UNUSED(tool);
  UNUSED(distance);
...
static void on_tool_tilt(void *data, struct zwp_tablet_tool_v2 *tool,
                         wl_fixed_t x, wl_fixed_t y) {
  UNUSED(data);
  UNUSED(tool);
  UNUSED(x);
...
static void on_tool_frame(void *data, struct zwp_tablet_tool_v2 *tool,
                          uint32_t time) {
  UNUSED(data);
  UNUSED(tool);
  UNUSED(time);
  ...
  if (env_pointer->frame.mask & EVENT_FRAME_PROXIMITY) {
    if (env_pointer->frame.mask & EVENT_FRAME_SURFACE) {
      environment_t *env =
          environment_from_surface(env_pointer->frame.surface_changed);
      if (!env) {
        return;
      }
      ...
      if (!env->select_active && is_root_surface(surface)) {
        // if
        // (!why_tablet_v2_proximity_in_events_received_by_parent_question_mark
        // && !disable_tablet_workarounds) {
        //     why_tablet_v2_proximity_in_events_received_by_parent_question_mark
        //     = true; WARN("WORKAROUND: zwp_tablet_v2 proximity_in events are
        //     received by parent in KDE, not by subsurface. Applying stupid
        //     workaround.");
        // }
        struct mascot *mascot = environment_mascot_by_coordinates(
            env, wl_fixed_to_int(env_pointer->frame.surface_local_x),
            wl_fixed_to_int(env_pointer->frame.surface_local_y));
        if (mascot) {
...
static void on_tablet_added(void *data, struct zwp_tablet_seat_v2 *tablet_seat,
...
static void on_tool_added(void *data, struct zwp_tablet_seat_v2 *tablet_seat,
                          struct zwp_tablet_tool_v2 *tool) {
  UNUSED(data);
  UNUSED(tablet_seat);

  environment_pointer_t *env_pointer =
      allocate_env_pointer(ENVIRONMENT_POINTER_PROTOCOL_TABLET, tool);
  zwp_tablet_tool_v2_add_listener(tool, &zwp_tablet_tool_v2_listener,
                                  env_pointer);
}

static void on_pad_added(void *data, struct zwp_tablet_seat_v2 *tablet_seat,
...
static void handle_fractional_scale_manager(void *data, uint32_t id,
                                            uint32_t version) {
  UNUSED(data);
  DEBUG("Binded wp_viewporter global of ver %u", version);
...
static void handle_tablet_manager(void *data, uint32_t id, uint32_t version) {
  ...
  tablet_manager =
      wl_registry_bind(registry, id, &zwp_tablet_manager_v2_interface, version);
}
...
static void handle_output(void *data, uint32_t id, uint32_t version) {
  ...
  if (new_environment) {
    ...
    if (!env)
      ERROR("OOM CONDITION while in handle_output");
...
static void handle_cursor_shape(void *data, uint32_t id, uint32_t version) {
  UNUSED(data);
  cursor_shape_manager = wl_registry_bind(
      registry, id, &wp_cursor_shape_manager_v1_interface, version);
  DEBUG("Binded cursor_shape global of ver %u", version);
}

static void handle_xdg_output_manager(void *data, uint32_t id,
                                      uint32_t version) {
  UNUSED(data);
...
static void handle_alpha_modifier_v1(void *data, uint32_t id,
                                     uint32_t version) {
  UNUSED(data);
  alpha_modifier_manager =
      wl_registry_bind(registry, id, &wp_alpha_modifier_v1_interface, version);
...
static void on_global(void *data, struct wl_registry *registry, uint32_t id,
  ...
  if (!strcmp(iface_name, wl_compositor_interface.name)) {
    ...
    handle_wlr_layer_shell(data, id, version);
             ...
             !(envs->flags & ENV_DISABLE_TABLETS)) {
...
environment_init(int flags, void (*new_listener)(environment_t *),
                 void (*rem_listener)(environment_t *),
  ...
  if (session) {
    if (!strcmp(session, "Hyprland")) {
      WARN(
          "Hyprland session detected, applying workaround for mascot dragging");
      we_on_hyprland = true;
    }
  }
  ...
  if (!display) {
    snprintf(error_m, 1024,
             "Environment initialization failed: Failed to connect to Wayland "
             "display");
  ...
  struct envs_queue *envs =
      (struct envs_queue *)calloc(1, sizeof(struct envs_queue));
  if (!envs)
    ERROR("OOM CONDITION while in environment_init");
  envs->flags = flags;
  ...
  if (!compositor) {
    error_offt +=
        snprintf(error_m, 1024, "Required global is missing: wl_compositor.\n");
    init_status = ENV_INIT_ERROR_GLOBALS;
  }
  if (!shm_manager) {
    error_offt += snprintf(error_m + error_offt, 1024 - error_offt,
                           "Required global is missing: wl_shm.\n");
    init_status = ENV_INIT_ERROR_GLOBALS;
  }
  if (!subcompositor) {
    error_offt += snprintf(
        error_m + error_offt, 1024 - error_offt,
        "Required global is missing: wl_subcompositor\nNote: Following global "
        "is crucial for the correct operation of the program.\n");
    init_status = ENV_INIT_ERROR_GLOBALS;
  }
  if (!wlr_layer_shell) {
    error_offt +=
        snprintf(error_m + error_offt, 1024 - error_offt,
                 "Required protocol is missing: wlr_layer_shell.\nNote: "
                 "Following global is base of the program.\n");
    init_status = ENV_INIT_ERROR_GLOBALS;
  }
  ...
  if (!empty_region) {
    snprintf(
        error_m, 1024,
        "Environment initialization failed: Failed to create empty region");
    init_status = ENV_INIT_ERROR_GENERIC;
    return ENV_INIT_ERROR_GENERIC;
  }
  ...
  if (tablet_manager && !(flags & ENV_DISABLE_TABLETS)) {
    ...
    if (tablet_seat) {
      zwp_tablet_seat_v2_add_listener(tablet_seat, &zwp_tablet_seat_v2_listener,
                                      NULL);
    } else {
...
int environment_dispatch() { return wl_display_dispatch(display); }
  ...
  for (uint32_t i = 0; i < list_size(env->mascot_manager.referenced_mascots);
    ...
    if (mascot) {
      if (orphaned_mascot) {
        ...
        WARN("<Mascot:%s:%u> Can not find new environment for orphaned mascot, "
             "dismissing",
...
environment_subsurface_t *environment_create_subsurface(environment_t *env) {
  ...
  environment_subsurface_t *subsurface =
      (environment_subsurface_t *)calloc(1, sizeof(environment_subsurface_t));
  if (!subsurface)
    return NULL;
  ...
  wl_surface_set_data(subsurface->surface, WL_SURFACE_ROLE_SUBSURFACE,
                      subsurface, env);

  subsurface->subsurface = wl_subcompositor_get_subsurface(
      subcompositor, subsurface->surface, env->root_surface->surface);
  ...
  if (viewporter) {
    subsurface->viewport =
        wp_viewporter_get_viewport(viewporter, subsurface->surface);
  ...
  if (alpha_modifier_manager) {
    subsurface->alpha_modifier = wp_alpha_modifier_v1_get_surface(
        alpha_modifier_manager, subsurface->surface);
    wp_alpha_modifier_surface_v1_set_multiplier(
        subsurface->alpha_modifier,
        (uint32_t)((double)config_get_opacity() * (double)UINT32_MAX));
  }
...
void environment_subsurface_attach(environment_subsurface_t *surface,
                                   const struct mascot_pose *pose) {
  if (!surface->surface)
    return;
  if (!pose) {
  ...
  struct mascot_sprite *sprite =
      pose->sprite[surface->mascot->LookingRight->value.i];
  if (!sprite) {
    environment_subsurface_unmap(surface);
    return;
  }
  ...
  if (!surface->drag_pointer && config_get_dragging()) {
    environment_buffer_scale_input_region(
        sprite->buffer, (config_get_mascot_scale() * surface->env->scale));
  ...
  if (!surface->pose) {
    wl_subsurface_place_below(
        surface->subsurface,
        surface->env->root_environment_subsurface->surface);
    if (surface->mascot) {
      environment_subsurface_set_position(
          surface, surface->mascot->X->value.i,
          yconv(surface->env, surface->mascot->Y->value.i));
...
environment_subsurface_move(environment_subsurface_t *surface, int32_t dx,
                            int32_t dy, bool use_callback,
  ...
  collision = check_movement_collision(
      &surface->env->workarea_geometry, current_x, current_y, dx,
      yconv(surface->env, dy), BORDER_TYPE_NONE, &proposed_x, &proposed_y);

  if ((MOVE_OOB(collision) ||
       is_outside(&surface->env->workarea_geometry, current_x, current_y)) &&
      (config_get_allow_throwing_multihead() || config_get_unified_outputs())) {
    ...
    if (new_env == surface->env) {
      ...
      mascot_moved(surface->mascot, proposed_x,
                   yconv(surface->env, proposed_y));
      return environment_move_ok;
    } else if (new_env) {
      int32_t diff_x, diff_y;
      environment_global_coordinates_delta(surface->env, new_env, &diff_x,
                                           &diff_y);
      proposed_x = dx;
      proposed_y = dy;
      ...
      mascot_apply_environment_position_diff(surface->mascot, diff_x, diff_y,
                                             move_flags, new_env);
      mascot_environment_changed(surface->mascot, new_env);

      if (is_outside(&new_env->workarea_geometry, surface->mascot->X->value.i,
                     surface->mascot->Y->value.i)) {
        if (surface->mascot->X->value.i < new_env->workarea_geometry.x) {
          surface->mascot->X->value.i = new_env->workarea_geometry.x;
        } else if (surface->mascot->X->value.i >
                   new_env->workarea_geometry.x +
                       new_env->workarea_geometry.width) {
          surface->mascot->X->value.i =
              new_env->workarea_geometry.x + new_env->workarea_geometry.width;
        }
        if (surface->mascot->Y->value.i < new_env->workarea_geometry.y) {
          surface->mascot->Y->value.i = new_env->workarea_geometry.y;
        } else if (surface->mascot->Y->value.i >
                   new_env->workarea_geometry.y +
                       new_env->workarea_geometry.height) {
          surface->mascot->Y->value.i =
              new_env->workarea_geometry.y + new_env->workarea_geometry.height;
        }
      }
  ...
  if (COLLIDED(collision) &&
      !environment_neighbor_border(surface->env, proposed_x, proposed_y) &&
      surface->mascot->state != mascot_state_jump) {
    dx = proposed_x;
  ...
  if (environment_ie_is_active() && !COLLIDED(collision)) {
    ...
    collision = check_movement_collision(
        &bb, current_x, current_y, dx, yconv(surface->env, dy),
        BORDER_TYPE_FLOOR, &proposed_x, &proposed_y);
  ...
  if (!use_interpolation) {
    if (environment_subsurface_set_position(surface, dx, dy) ==
...
environment_t *
environment_subsurface_get_environment(environment_subsurface_t *surface) {
  if (!surface)
    return NULL;
  return surface->env;
}

// bool environment_subsurface_move_to_pointer(environment_subsurface_t*
// surface);

struct wl_compositor *environment_get_compositor() { return compositor; }
struct wl_shm *environment_get_shm() { return shm_manager; }

void environment_subsurface_drag(environment_subsurface_t *surface,
                                 environment_pointer_t *pointer) {
  if (!surface)
    return;
  if (!surface->surface)
  ...
  wl_surface_set_input_region(surface->surface, empty_region);
  wl_subsurface_place_above(surface->subsurface,
                            surface->env->root_environment_subsurface->surface);
...
void environment_subsurface_release(environment_subsurface_t *surface) {
  ...
  wl_surface_set_input_region(
      surface->surface,
      surface->pose->sprite[surface->mascot->LookingRight->value.i]
          ->buffer->region);
  wl_subsurface_place_below(surface->subsurface,
                            surface->env->root_environment_subsurface->surface);
  ...
  if (!config_get_unified_outputs() && config_get_allow_dragging_multihead()) {
    ...
    if (is_outside(&surface->env->workarea_geometry, global_x, global_y)) {
      ...
      if (new_env) {
        ...
        mascot_apply_environment_position_diff(
            surface->mascot, diff_x, diff_y,
            DIFF_HORIZONTAL_MOVE | DIFF_VERTICAL_MOVE, new_env);
        mascot_environment_changed(surface->mascot, new_env);
      }
    }
  }
...
void environment_pointer_update_delta(environment_subsurface_t *subsurface,
                                      uint32_t tick) {
  if (!subsurface)
    return;
  if (!subsurface->drag_pointer)
    return;
...
bool environment_subsurface_move_to_pointer(environment_subsurface_t *surface) {
  ...
  environment_subsurface_set_position(surface, surface->drag_pointer->x,
                                      surface->drag_pointer->y);
  if (surface->mascot)
    mascot_moved(surface->mascot, surface->drag_pointer->x,
                 yconv(surface->env, surface->drag_pointer->y));
...
int32_t environment_cursor_x(struct mascot *mascot, environment_t *env) {
  UNUSED(env);
  environment_pointer_t *pointer =
      mascot ? mascot->subsurface->drag_pointer : active_pointer;
  if (!pointer)
    pointer = active_pointer;
  if (!pointer)
    return 0;

  return pointer->grabbed_subsurface
             ? pointer->x
             : pointer->public_x - env->global_geometry.x;
}
...
int32_t environment_cursor_y(struct mascot *mascot, environment_t *env) {
  ...
  return pointer->grabbed_subsurface
             ? pointer->y
             : pointer->public_y - env->global_geometry.y;
}
...
int32_t environment_cursor_get_tick_diff(environment_pointer_t *pointer,
                                         uint32_t tick) {
...
void enviroment_wait_until_ready(environment_t *env) {
  while (!env->is_ready) {
    ...
    if (environment_dispatch() == -1) {
      ERROR("Failed to dispatch Wayland events! error code: %d",
...
void environment_subsurface_associate_mascot(environment_subsurface_t *surface,
                                             struct mascot *mascot_ptr) {
  if (!surface)
    return;
  surface->mascot = mascot_ptr;
}

void environment_subsurface_set_offset(environment_subsurface_t *surface,
                                       int32_t x, int32_t y) {
  if (!surface)
    return;
  surface->offset_x = x;
...
void environment_select_position(environment_t *env,
                                 void (*callback)(environment_t *, int32_t,
                                                  int32_t,
                                                  environment_subsurface_t *,
                                                  void *),
                                 void *data) {
  ...
  if (env->select_active)
    environment_pointer_apply_cursor(active_pointer,
...
int environment_get_display_fd() { return wl_display_get_fd(display); }

struct mascot *
environment_subsurface_get_mascot(environment_subsurface_t *surface) {
  if (!surface)
    return NULL;
  return surface->mascot;
}

const struct mascot_pose *
environment_subsurface_get_pose(environment_subsurface_t *surface) {
  if (!surface)
    return NULL;
  return surface->pose;
}
...
const char *environment_get_error() {
  if (init_status == ENV_NOT_INITIALIZED)
    return "environment_get_error() called before initialization attempt";
  if (init_status == ENV_INIT_OK)
    return "No error";
  return error_m;
}

void environment_set_public_cursor_position(environment_t *env, int32_t x,
                                            int32_t y) {
  UNUSED(env);
  if (!active_pointer)
    return;
...
environment_buffer_t *environment_shm_pool_create_buffer(
    environment_shm_pool_t *pool, uint32_t offset, uint32_t width,
  ...
  buffer->buffer = wl_shm_pool_create_buffer(pool->pool, offset, width, height,
                                             stride, format);
  buffer->width = width;
...
environment_popup_t *environment_popup_create(environment_t *environment,
                                              struct mascot *mascot, uint32_t x,
                                              uint32_t y, uint32_t width,
                                              uint32_t height) {
  if (!environment || !mascot)
    return NULL;
  ...
  popup->popup =
      xdg_surface_get_popup(popup->xdg_surface, NULL, popup->position);
  zwlr_layer_surface_v1_get_popup(environment->root_surface->layer_surface,
                                  popup->popup);

  struct environment_callbacks *callbacks =
      calloc(1, sizeof(struct environment_callbacks));
...
void environment_get_output_id_info(environment_t *env, const char **name,
                                    const char **make, const char **model,
                                    const char **desc, uint32_t *id) {
  if (!env)
    return;
  if (name)
    *name = env->output.name;
  if (make)
    *make = env->output.make;
  if (model)
    *model = env->output.model;
  if (desc)
    *desc = env->output.desc;
  if (id)
    *id = env->output.id;
}

enum environment_move_result
environment_subsurface_set_position(environment_subsurface_t *surface,
  ...
  if (surface->pose) {
    surface_anchor_x += (surface->mascot->LookingRight->value.i
                             ? -surface->width - surface->pose->anchor_x
                             : surface->pose->anchor_x);
    surface_anchor_y += surface->pose->anchor_y;
  }

  surface_anchor_x = (float)surface_anchor_x /
                     (config_get_mascot_scale() * surface->env->scale);
  surface_anchor_y = (float)surface_anchor_y /
                     (config_get_mascot_scale() * surface->env->scale);

  wl_subsurface_set_position(surface->subsurface, dx + surface_anchor_x,
                             dy + surface_anchor_y);
...
int32_t environment_workarea_width(environment_t *env) {
  return env->advertised_geometry.width ? env->advertised_geometry.width
                                        : (int32_t)env->width;
}

int32_t environment_workarea_height(environment_t *env) {
  return env->advertised_geometry.height
             ? env->advertised_geometry.height
             : (int32_t)env->workarea_geometry.height;
}
...
uint32_t environment_id(environment_t *env) { return env->id; }
...
bool environment_logical_position(environment_t *env, int32_t *lx,
                                  int32_t *ly) {
  if (!env->xdg_output_done)
    return false;
  *lx = env->lx;
  *ly = env->ly;
  return true;
...
environment_get_border_type_rect(environment_t *env, int32_t x, int32_t y,
                                 struct bounding_box *rect,
                                 int32_t border_mask) {
  int32_t border_type = BORDER_TYPE(check_collision_at(rect, x, y, 0));
  int32_t masked_border = APPLY_MASK(border_type, border_mask);

  if (masked_border & BORDER_TYPE_CEILING)
    return (rect->type == INNER_COLLISION ? environment_border_type_ceiling
                                          : environment_border_type_floor);
  if (BORDER_IS_WALL(masked_border))
    return environment_border_type_wall;
  if (masked_border & BORDER_TYPE_FLOOR)
    return (rect->type == INNER_COLLISION ? environment_border_type_floor
                                          : environment_border_type_ceiling);

  if (environment_neighbor_border(env, x, y))
    return environment_border_type_none;

  if (border_type & BORDER_TYPE_CEILING)
    return (rect->type == INNER_COLLISION ? environment_border_type_ceiling
                                          : environment_border_type_floor);
  if (BORDER_IS_WALL(border_type))
    return environment_border_type_wall;
  if (border_type & BORDER_TYPE_FLOOR)
    return (rect->type == INNER_COLLISION ? environment_border_type_floor
                                          : environment_border_type_ceiling);
...
enum environment_border_type environment_get_border_type(environment_t *env,
                                                         int32_t x, int32_t y) {
  if (!env->is_ready)
  ...
  return environment_get_border_type_rect(env, x, y, &env->workarea_geometry,
                                          env->border_mask);
}
...
bool environment_migrate_subsurface(environment_subsurface_t *surface,
                                    environment_t *env) {
  if (!surface)
    return false;
  if (!env)
  ...
  if (mascot) {
    ...
    uint32_t mascot_index =
        list_find(surface->env->mascot_manager.referenced_mascots, mascot);
    if (mascot_index != UINT32_MAX) {
      list_remove(surface->env->mascot_manager.referenced_mascots,
                  mascot_index);
    }
  }
  ...
  if (!userdata)
    ERROR("Unexpected NULL userdata while in migration");
  ...
  surface->subsurface = wl_subcompositor_get_subsurface(
      subcompositor, surface->surface, env->root_surface->surface);
  if (!surface->subsurface)
    ERROR("Failed to create subsurface!");

  if (viewporter) {
    surface->viewport =
        wp_viewporter_get_viewport(viewporter, surface->surface);
    if (!surface->viewport)
      ERROR("Failed to create viewport!");
  }

  if (alpha_modifier_manager) {
    surface->alpha_modifier = wp_alpha_modifier_v1_get_surface(
        alpha_modifier_manager, surface->surface);
    if (!surface->alpha_modifier)
      ERROR("Failed to create alpha modifier!");
  }
...
void environment_disable_tablet_workarounds(bool value) {
  ...
  if (disable_tablet_workarounds &
      why_tablet_v2_proximity_in_events_received_by_parent_question_mark) {
    WARN("Tablet v2 proximity_in events received by parent workaround is "
         "disabled");
    why_tablet_v2_proximity_in_events_received_by_parent_question_mark = false;
  }
}

const char *environment_get_backend_name() { return "Wayland"; }

environment_buffer_factory_t *environment_buffer_factory_new() {
  if (!compositor)
    ERROR("Failed to create buffer factory: wl_compositor is not available");
  if (!shm_manager)
    ERROR("Failed to create buffer factory: wl_shm is not available");
  environment_buffer_factory_t *factory =
      (environment_buffer_factory_t *)calloc(
          1, sizeof(environment_buffer_factory_t));
  if (!factory)
    ERROR("Failed to create buffer factory: Out of memory");

  factory->memfd = memfd_create("environment_factory_buffer",
...
void environment_buffer_factory_destroy(environment_buffer_factory_t *factory) {
  if (!factory)
    return;
  if (factory->pool)
    wl_shm_pool_destroy(factory->pool);
  close(factory->memfd);
  free(factory);
}

bool environment_buffer_factory_write(environment_buffer_factory_t *factory,
...
void environment_buffer_factory_done(environment_buffer_factory_t *factory) {
  ...
  factory->pool =
      wl_shm_create_pool(shm_manager, factory->memfd, factory->size);
  if (!factory->pool)
    ERROR("Failed to create buffer factory pool");

  factory->done = true;
}

environment_buffer_t *
environment_buffer_factory_create_buffer(environment_buffer_factory_t *factory,
                                         int32_t width, int32_t height,
  ...
  environment_buffer_t *buffer =
      (environment_buffer_t *)calloc(1, sizeof(environment_buffer_t));
  if (!buffer)
    ERROR("Failed to create buffer: Out of memory");

  buffer->buffer = wl_shm_pool_create_buffer(
      factory->pool, offset, width, height, stride, WL_SHM_FORMAT_ARGB8888);
  if (!buffer->buffer) {
    free(buffer);
...
void environment_buffer_add_to_input_region(environment_buffer_t *buffer,
...
void environment_buffer_subtract_from_input_region(environment_buffer_t *buffer,
                                                   int32_t x, int32_t y,
                                                   int32_t width,
                                                   int32_t height) {
  if (!buffer)
  ...
  if (!buffer->region) {
    buffer->region = wl_compositor_create_region(compositor);
    if (!buffer->region) {
      WARN("Failed to subtract buffer from input region: Failed to create "
           "region");
      return;
    }
    buffer->scale_factor = 1;
...
void environment_buffer_scale_input_region(environment_buffer_t *buffer,
                                           float scale) {
  if (!buffer)
    return;
  if (!buffer->buffer)
  ...
  wl_region_add(buffer->region, buffer->input_region_desc.x / scale,
                buffer->input_region_desc.y / scale,
                buffer->input_region_desc.width / scale,
                buffer->input_region_desc.height / scale);
...
uint64_t environment_interpolate(environment_t *env) {
  ...
  if (framerate < 0)
    framerate = (env->output.refresh / 1000.0);
  if (framerate < 1.0)
    return 0;
  ...
  pthread_mutex_lock(&env->mascot_manager.mutex);
  for (uint32_t i = 0, c = list_count(env->mascot_manager.referenced_mascots);
       i < list_size(env->mascot_manager.referenced_mascots) && c > 0; i++) {
    struct mascot *mascot = list_get(env->mascot_manager.referenced_mascots, i);
    if (mascot) {
      c--;
      if (mascot->subsurface) {
        if (mascot->subsurface->interpolation_data.x ==
                mascot->subsurface->interpolation_data.new_x &&
            mascot->subsurface->interpolation_data.y ==
                mascot->subsurface->interpolation_data.new_y) {
          continue;
        }

        if (mascot->subsurface->is_grabbed)
          continue;
        float delta_x = mascot->subsurface->interpolation_data.new_x -
                        mascot->subsurface->interpolation_data.prev_x;
        float delta_y = mascot->subsurface->interpolation_data.new_y -
                        mascot->subsurface->interpolation_data.prev_y;
        ...
        if (delta_x > 0 && new_x > mascot->subsurface->interpolation_data.new_x)
          new_x = mascot->subsurface->interpolation_data.new_x;
        if (delta_x < 0 && new_x < mascot->subsurface->interpolation_data.new_x)
          new_x = mascot->subsurface->interpolation_data.new_x;
        if (delta_y > 0 && new_y > mascot->subsurface->interpolation_data.new_y)
          new_y = mascot->subsurface->interpolation_data.new_y;
        if (delta_y < 0 && new_y < mascot->subsurface->interpolation_data.new_y)
          new_y = mascot->subsurface->interpolation_data.new_y;

        environment_subsurface_set_position(mascot->subsurface, round(new_x),
                                            round(new_y));
...
void environment_subsurface_reset_interpolation(
    environment_subsurface_t *subsurface) {
  if (!subsurface)
    return;
...
void environment_subsurface_scale_coordinates(environment_subsurface_t *surface,
                                              int32_t *x, int32_t *y) {
  if (!surface)
    return;
  if (!x || !y)
      ...
      for (size_t j = 0; j < result.events_count; j++) {
        ...
        if (event_type == mascot_tick_clone) {
          ...
          if (clone) {
            ...
            mascot_attach_affordance_manager(
                clone, environment->mascot_manager.affordances);
            mascot_link(clone);
...
void environment_remove_mascot(environment_t *environment,
                               struct mascot *mascot) {
  if (!environment)
    return;
  if (!mascot)
  ...
  if (mascot->prototype->dismiss_action &&
  ...
  if (res != ACTION_SET_RESULT_OK) {
    uint32_t index =
        list_find(environment->mascot_manager.referenced_mascots, mascot);
    if (index != UINT32_MAX) {
...
void environment_set_prototype_store(environment_t *environment,
                                     mascot_prototype_store *store) {
  if (!environment)
    return;
  if (!store)
    return;
...
void environment_summon_mascot(environment_t *environment,
                               struct mascot_prototype *prototype, int32_t x,
                               int32_t y,
                               void (*callback)(struct mascot *, void *),
                               void *data) {
  if (!environment)
    return;
  if (!prototype)
    return;
  struct mascot *mascot = mascot_new(prototype, NULL, 0, 0, x, y, 2.0, 0.05,
                                     0.1, false, environment);
  if (!mascot) {
    if (callback)
      callback(mascot, data);
    return;
  }
  ...
  mascot_attach_affordance_manager(mascot,
                                   environment->mascot_manager.affordances);
  mascot_link(mascot);
  pthread_mutex_unlock(&environment->mascot_manager.mutex);
...
void environment_global_coordinates_delta(environment_t *environment_a,
  ...
  // This is useful for calculating the new position of a mascot when moving it
...
struct mascot *environment_mascot_by_coordinates(environment_t *environment,
                                                 int32_t x, int32_t y) {
  if (!environment)
    return NULL;
  if (!environment->is_ready)
    return NULL;
  if (!environment->mascot_manager.referenced_mascots)
    return NULL;
  ...
  for (size_t i = 0, c = 0; i < list_size(mascots) && c < list_count(mascots);
    ...
    if (mascot_->subsurface) {
      ...
      if (mascot_->subsurface->pose) {
        ...
        if (mascot_->subsurface->pose) {
          surface_anchor_x +=
              (mascot_->subsurface->mascot->LookingRight->value.i
                   ? -mascot_->subsurface->width -
                         mascot_->subsurface->pose->anchor_x
                   : mascot_->subsurface->pose->anchor_x);
          surface_anchor_y += mascot_->subsurface->pose->anchor_y;
        }

        surface_anchor_x =
            (float)surface_anchor_x /
            (config_get_mascot_scale() * mascot_->subsurface->env->scale);
        surface_anchor_y =
            (float)surface_anchor_y /
            (config_get_mascot_scale() * mascot_->subsurface->env->scale);

        struct mascot_sprite *sprite =
            mascot_->subsurface->pose->sprite[mascot_->LookingRight->value.i];
      ...
      INFO("Checking hitbox: %d %d %d %d, pointer at (%d, %d)", hitbox.x,
           hitbox.y, hitbox.width, hitbox.height, x, y);
      ...
      // if (x >= mascot_->subsurface->x && x <= mascot_->subsurface->x +
      // mascot_->subsurface->width) {
...
struct mascot *environment_mascot_by_id(environment_t *environment,
                                        uint32_t id) {
  if (!environment)
    return NULL;
  if (!environment->is_ready)
...
struct list *environment_mascot_list(environment_t *environment,
                                     pthread_mutex_t **list_mutex) {
  if (!environment)
    return NULL;

  if (list_mutex)
    *list_mutex = &environment->mascot_manager.mutex;
...
void environment_to_global_coordinates(environment_t *environment, int32_t *x,
                                       int32_t *y) {
  if (!environment)
    return;
  if (!x || !y)
...
void environment_recalculate_advertised_geometry(environment_t *env) {
  ...
  int32_t max_left_offset = 0, max_right_offset = 0, max_top_offset = 0,
          max_bottom_offset = 0;
  int32_t max_width = env->workarea_geometry.width,
          max_height = env->workarea_geometry.height;

  int32_t new_border_mask = 0;
  bool ceiling_aligned = false, floor_aligned = false, left_aligned = false,
       right_aligned = false;
  ...
  for (size_t i = 0; i < list_size(env->neighbors); i++) {
    ...
    if (!neighbor)
      continue;
    if (!neighbor->is_ready)
      continue;

    // Check if the neighbor touches the left border
    if (bounding_box_touches(&env->global_geometry, &neighbor->global_geometry,
                             BORDER_TYPE_RIGHT | BORDER_TYPE_CEILING |
                                 BORDER_TYPE_FLOOR)) {
      int32_t left_offset =
          env->global_geometry.x - neighbor->global_geometry.x;
      if (left_offset > max_left_offset) {
        max_left_offset = left_offset;
        max_width =
            neighbor->global_geometry.width + env->global_geometry.width;
      }
      new_border_mask |= BORDER_TYPE_LEFT;
      if (neighbor->global_geometry.y == env->global_geometry.y) {
        ceiling_aligned = true;
      }
      if (neighbor->global_geometry.y + neighbor->global_geometry.height ==
          env->global_geometry.y + env->global_geometry.height) {
        floor_aligned = true;
      }
      continue;
    }
    ...
    if (bounding_box_touches(&env->global_geometry, &neighbor->global_geometry,
                             BORDER_TYPE_LEFT | BORDER_TYPE_CEILING |
                                 BORDER_TYPE_FLOOR)) {
      int32_t right_offset = neighbor->global_geometry.x +
                             neighbor->global_geometry.width -
                             env->global_geometry.x;
    ...
    if (bounding_box_touches(&env->global_geometry, &neighbor->global_geometry,
                             BORDER_TYPE_RIGHT | BORDER_TYPE_CEILING |
                                 BORDER_TYPE_LEFT)) {
      int32_t bottom_offset = neighbor->global_geometry.y +
                              neighbor->global_geometry.height -
                              env->global_geometry.y;
  ...
  INFO("------------[Environment %d recalculate advertised "
       "geometry]---------------",
       env->id);
  INFO("New border mask is %d", env->border_mask);
  INFO("Advertised geometry is %d, %d, %d, %d", env->advertised_geometry.x,
       env->advertised_geometry.y, env->advertised_geometry.width,
       env->advertised_geometry.height);
  INFO("Geometry alignment: %s%s%s%s", ceiling_aligned ? "ceiling " : "",
       floor_aligned ? "floor " : "", left_aligned ? "left " : "",
       right_aligned ? "right" : "");
  INFO("-----------------------------------------------------------------------"
       "---");
}

void environment_announce_neighbor(environment_t *environment,
                                   environment_t *neighbor) {
  if (!environment || !neighbor)
    return;
...
void environment_widthdraw_neighbor(environment_t *environment,
                                    environment_t *neighbor) {
  if (!environment || !neighbor)
    return;
  if (!config_get_unified_outputs())
...
bool environment_neighbor_border(environment_t *environment, int32_t x,
                                 int32_t y) {
  if (!environment)
    return false;
  if (!environment->is_ready)
  ...
  for (size_t i = 0; i < list_size(environment->neighbors); i++) {
    ...
    if (!neighbor)
      continue;
    int32_t collision_at = BORDER_TYPE(
        check_collision_at(&neighbor->global_geometry, global_x, global_y, 0));
    if (collision_at)
      return true;
  }
  return false;
...
void environment_set_affordance_manager(
    environment_t *environment, struct mascot_affordance_manager *manager) {
  if (!environment)
    return;
  environment->mascot_manager.affordances = manager;
  for (size_t i = 0;
       i < list_size(environment->mascot_manager.referenced_mascots); i++) {
    struct mascot *mascot =
        list_get(environment->mascot_manager.referenced_mascots, i);
    if (!mascot)
      continue;
...
int32_t environment_workarea_coordinate_aligned(environment_t *env,
                                                int32_t border_type,
                                                int32_t alignment_type) {
  if (!env)
    return 0;
  if (!env->is_ready)
    return 0;
...
int32_t environment_workarea_width_aligned(environment_t *env,
                                           int32_t alignment_type) {
  if (!env)
    return 0;
  if (!env->is_ready)
...
int32_t environment_workarea_height_aligned(environment_t *env,
                                            int32_t alignment_type) {
  if (!env)
    return 0;
  if (!env->is_ready)
...
void environment_recalculate_ie_attachement(environment_t *env, bool is_active,
                                            struct bounding_box new_geometry) {
  if (!env || !env->is_ready)
    return;
  ...
  for (uint32_t i = 0; i < env->mascot_manager.referenced_mascots->entry_count;
       i++) {
    struct mascot *mascot = list_get(env->mascot_manager.referenced_mascots, i);
    if (!mascot)
      continue;
    ...
    if (mascot_is_on_ie_left(mascot) &&
        (active_ie.geometry.x != environment_workarea_left(env))) {
      if (x != new_geometry.x) {
    ...
    } else if (mascot_is_on_ie_right(mascot) &&
               (active_ie.geometry.x + active_ie.geometry.width !=
                environment_workarea_right(env))) {
    ...
    } else if (mascot_is_on_ie_top(mascot) &&
               (active_ie.geometry.y != environment_workarea_top(env))) {
      if (y != new_geometry.y) {
    ...
    } else if (mascot_is_on_ie_bottom(mascot) &&
               (active_ie.geometry.y + active_ie.geometry.height !=
                environment_workarea_bottom(env))) {
...
struct bounding_box environment_get_active_ie(environment_t *environment) {
  if (!environment)
    return (struct bounding_box){0};
  if (!environment->is_ready)
    return (struct bounding_box){0};
  if (!active_ie.is_active)
    return (struct bounding_box){0};

  if (!config_get_unified_outputs()) {
    if (!bounding_boxes_intersect(&environment->global_geometry,
                                  &active_ie.geometry, NULL))
      return (struct bounding_box){0};
  }
...
bool environment_ie_is_active() { return active_ie.is_active; }
...
void environment_mascot_detach_ie_movers(environment_t *env) {
  ...
  for (uint32_t i = 0; i < env->mascot_manager.referenced_mascots->entry_count;
    ...
    if (mascot->state == mascot_state_ie_fall ||
