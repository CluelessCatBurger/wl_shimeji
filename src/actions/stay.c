/*
    stay.c - wl_shimeji's stay action implementation
...
enum mascot_tick_result stay_action_init(struct mascot *mascot, struct mascot_action_reference *actionref, uint32_t tick)
{
    if (!actionref->action->length) {
        WARN("<Mascot:%s:%u> Stay action has no length", mascot->prototype->name, mascot->id);
        return mascot_tick_error;
    }

    DEBUG("<Mascot:%s:%u> Initializing Stay action \"%s\"", mascot->prototype->name, mascot->id, actionref->action->name);
    DEBUG("<Mascot:%s:%u> Stay actionref info:", mascot->prototype->name, mascot->id);
