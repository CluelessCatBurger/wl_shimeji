/*
    dispose.c - wl_shimeji's dispose action implementation
...
enum mascot_tick_result dispose_action_init(struct mascot *mascot, struct mascot_action_reference *actionref, uint32_t tick)
{
    UNUSED(tick);

    DEBUG("<Mascot:%s:%u> Initializing dispose action \"%s\"", mascot->prototype->name, mascot->id, actionref->action->name);
    DEBUG("<Mascot:%s:%u> dispose actionref info:", mascot->prototype->name, mascot->id);
    ...
    mascot->state = mascot_state_animate;
...
struct mascot_action_next dispose_action_next(struct mascot* mascot, struct mascot_action_reference *actionref, uint32_t tick)
{
    struct mascot_action_next result = {0};

    if (!actionref->action->length) {
        result.status = mascot_tick_dispose;
        return result;
    }
